# -*- coding: utf-8 -*-
# 💸 Hisheb Bot Pro (Railway Final Stable v5)
# ✅ Fully Working: All Commands + Markdown + Emoji + CSV Backup + Reset
...
DB = "/tmp/expenses.db"  # Railway-safe DB path
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = (
        ...
        "Track your daily and monthly expenses effortlessly 💰\\n"
...
async def add_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ Added *{amt:.2f}* 💵 in *{cat}*\n📝 {escape_markdown(note or 'No note', version=2)}",
...
async def summary_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        await update.message.reply_text("📭 No records yet."); return
...
async def daily_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rows = c.fetchall(); conn.close()
    if not rows:
        await update.message.reply_text("📭 No records for today."); return
    total = sum(r[1] for r in rows)
    msg = "📅 *Today's Summary:*\n\n" + "\n".join([f"• {r[0]}: {r[1]:.2f} 💵" for r in rows])
    msg += f"\n\n💰 *Total:* {total:.2f}"
    await update.message.reply_text(msg, parse_mode="MarkdownV2")
...
async def chart_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rows = c.fetchall(); conn.close()
    if not rows:
        await update.message.reply_text("📭 No data to show chart."); return
...
async def export_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_document(update.effective_chat.id,
        ...
        caption="📤 Your monthly expense export")
...
async def reset_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        "Before deletion, a CSV backup will be sent.\n\n"
...
if __name__ == "__main__":
    print("🚀 Starting Hisheb Bot Pro (v5 Final Railway Build)…")
    app = build_app()
    app.run_polling()
