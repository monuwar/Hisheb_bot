# -*- coding: utf-8 -*-
# 💸 Hisheb Bot Pro (Railway Stable v4)
# Developer: Monuwar Hussain
# Fully working: add, summary, daily, monthly, chart, export, reset
...
DB = "/tmp/expenses.db"  # Railway-safe
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Use */commands* to see everything you can do.",
...
async def add_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = context.args
    if len(args)<2:
        await update.message.reply_text("❗ Usage: `/add <amount> <category> [note]`", parse_mode="MarkdownV2"); return
    try: amt=float(args[0])
    except ValueError:
        await update.message.reply_text("❌ Amount must be a number."); return
    cat=args[1]; note=" ".join(args[2:]) if len(args)>2 else ""
    conn=db_conn(); c=conn.cursor()
    c.execute("INSERT INTO expenses(user_id,amount,category,note,ts)VALUES(?,?,?,?,?)",
              (update.effective_user.id,amt,cat,note,now_ts())); conn.commit(); conn.close()
    await update.message.reply_text(
        f"✅ Added *{amt:.2f}* 💵 in *{cat}*\n📝 {note or 'No note'}",
        parse_mode="MarkdownV2")
...
async def summary_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    c.execute("SELECT category,SUM(amount)FROM expenses WHERE user_id=? GROUP BY category",
              (update.effective_user.id,)); rows=c.fetchall(); conn.close()
    if not rows: await update.message.reply_text("📭 No records yet."); return
...
async def daily_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    s,e=today_range(); conn=db_conn(); c=conn.cursor()
    c.execute("SELECT category,SUM(amount)FROM expenses WHERE user_id=? AND ts BETWEEN ? AND ? GROUP BY category",
              (update.effective_user.id,s,e)); rows=c.fetchall(); conn.close()
    if not rows: await update.message.reply_text("📭 No records for today."); return
    total=sum(r[1]for r in rows)
    msg="📅 *Today's Summary:*\n\n"+"\n".join([f"• {r[0]}: {r[1]:.2f} 💵"for r in rows])
    msg+=f"\n\n💰 *Total:* {total:.2f}"
    await update.message.reply_text(msg, parse_mode="MarkdownV2")
...
async def monthly_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    s,e=month_range(); conn=db_conn(); c=conn.cursor()
    c.execute("SELECT category,SUM(amount)FROM expenses WHERE user_id=? AND ts BETWEEN ? AND ? GROUP BY category",
              (update.effective_user.id,s,e)); rows=c.fetchall(); conn.close()
    if not rows: await update.message.reply_text("📭 No records for this month."); return
    total=sum(r[1]for r in rows)
...
async def chart_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    c.execute("SELECT category,SUM(amount)FROM expenses WHERE user_id=? AND ts BETWEEN ? AND ? GROUP BY category",
              (update.effective_user.id,s,e)); rows=c.fetchall(); conn.close()
    if not rows: await update.message.reply_text("📭 No data to show chart."); return
    labels,values=zip(*rows)
    plt.figure(figsize=(5,5)); plt.pie(values,labels=labels,autopct="%1.1f%%",startangle=90)
    plt.title("💸 Monthly Expense Chart"); plt.savefig("/tmp/chart.png"); plt.close()
    with open("/tmp/chart.png","rb")as f:
        await update.message.reply_photo(f, caption="🥧 *Monthly Expense Breakdown*", parse_mode="MarkdownV2")
...
async def export_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    c.execute("SELECT amount,category,note,ts FROM expenses WHERE user_id=? AND ts BETWEEN ? AND ?",
              (update.effective_user.id,s,e)); rows=c.fetchall(); conn.close()
    if not rows: await update.message.reply_text("📭 No data to export."); return
    out=io.StringIO(); w=csv.writer(out)
    w.writerow(["Amount","Category","Note","Date"])
    for r in rows: w.writerow([r[0],r[1],r[2],datetime.fromtimestamp(r[3]).strftime("%Y-%m-%d")])
    out.seek(0)
    await context.bot.send_document(update.effective_chat.id,
        document=io.BytesIO(out.getvalue().encode()),
        filename="Hisheb_Export.csv", caption="📤 Your monthly expense export")

# ========== RESET ==========
pending_reset={}
async def reset_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid=update.effective_user.id; pending_reset[uid]=True
    kb=[[InlineKeyboardButton("❌ Cancel",callback_data="cancel_reset")]]
    markup=InlineKeyboardMarkup(kb)
    msg=("⚠️ *Warning: Data Reset*\n\n"
...
async def cancel_reset(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in pending_reset:
        pending_reset.pop(uid); await q.edit_message_text("❎ Reset canceled.")
    else: await q.edit_message_text("ℹ️ No active reset found.")
...
if __name__=="__main__":
    print("🚀 Starting Hisheb Bot Pro v4 (Railway Stable)…")
    app=build_app()
    app.run_polling()
